            ...
            <form wire:submit="save" novalidate>
                ...
                <div class="grid items-center divide-y divide-blue-200 gap-1 ">
                    <div class="flex gap-2 mb-1">
                        <div class="">
                            <label for="countries"
                                class="block mb-1 text-sm font-medium text-gray-900 dark:text-white">Solicitante</label>
                            <select id="countries" wire:model.live="solicitante_id"
                                class="select2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                <option value="">Seleccione la persona que le solicito el trabajo</option>

                                @foreach ($solicitantes as $solicitante)
                                <option value="{{ $solicitante->id }}">{{ $solicitante->nombre }} {{ $solicitante->apellido }}</option>
                            @endforeach
